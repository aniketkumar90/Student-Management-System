...
<div class="info">
    <h3>All Rejected Students</h3>

    <?php include_once 'php_database/config_db.php'; ?>

    <?php
    $sql = "SELECT * FROM `sms_lsc` . `users` WHERE `users` . `user_type` = 2 and `users` . `status` = 3";
    ...
    <?php } else {
        echo "<div>
        <h3 style='color:red; margin-top: 18%;'>No Any Rejected Students Data Found!</h3>
        </div>";
    }
    ?>
</div>